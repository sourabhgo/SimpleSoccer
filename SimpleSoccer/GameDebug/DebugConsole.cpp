    ...
    break;
      ...
      PAINTSTRUCT ps;
        ...
        if (m_Buffer.size() > 1)
        {
          ...
          {
            TextOutA(ps.hdc, 0, cyChar*line++, (*beg).c_str(), (*beg).size()); 
          }
        }
...
bool DebugConsole::Create()
{
  ...
  {
    MessageBoxA(NULL, "Registration of Debug Console Failed!", "Error", 0);
 ...
 // GetClientRect(GetActiveWindow(), &rectActive);
  ...
  if(!m_hwnd)
  {
    MessageBoxA(m_hwnd, "CreateWindowEx Failed!", "Error!", 0);
